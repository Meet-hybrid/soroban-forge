# @soroban-forge/marketplace-royalties-client

Generated TypeScript client for the **Soroban Forge marketplace royalties
contract**: per-collection royalty configuration, settlement snapshots, and
...
```ts
import { Client } from "@soroban-forge/marketplace-royalties-client";
...
await client.set_royalty({ collection, recipient, bps });
const summary = await client.get_settlement_summary({ collection });
```
...
`stellar contract bindings typescript --wasm … --output-dir packages/marketplace-royalties-client --overwrite`.